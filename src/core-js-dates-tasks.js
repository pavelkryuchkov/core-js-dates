...
function getNextFriday(date) {
  const days = date.getDay() < 5 ? 5 - date.getDay() : 7 - (date.getDay() - 5);
...
function getWeekNumberByDate(date) {
  ...
  const days =
    (date - firstDate) / (1000 * 60 * 60 * 24) + firstDate.getDay() + 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const countWorkWeek = countWorkDays + countOffDays;
  const res = [];
  let current = 0;
  while (startDate <= endDate) {
    if (current < countWorkDays) {
      ...
      res.push(`${date}-${month}-${year}`);
    }
    current = (current + 1) % countWorkWeek;
    startDate.setDate(startDate.getDate() + 1);
  }
  return res;
}
